...
if test $hour -ge $NIGHT_START		# Night time check
	...
	curl -H "Title: $(hostname)" -H "Tags: control_knobs"  -d "$(hostname) is igoing to bed. Setting new startup time for next morning $(date +"%a %d %b %H:00:00 %Z %Y" -d +"$NIGHT_PERIOD"hours)" ntfy.sh/$(hostname)
